# =====================================================================
#  fusion-python-3.10_persistent.py — PERSISTENT-TRACKING VARIANT (copy of fusion-python-3.10.py)
#
#  camera (YOLO / YOLO-World, ByteTrack tracks) + radar (iwr1642_live_persistent.Pipeline, EKF tracks)
...
#
#  Objects no longer disappear the moment a frame is missed. Three changes from the original file,
#  all in service of one rule — "don't vanish unless truly out of frame or too far/small to trust":
#    1. Uses iwr1642_live_persistent + radar_filter_persistent (much longer track-coast budget,
#       explicit "too far" range cutoff — see those files).
#    2. HOLD_MIN_BBOX_PX: a radar-held box that has scaled down below this height (object walked
#       far enough away that the extrapolated box is basically a dot) stops being held.
#    3. CamOnlyHold: camera-only detections (no radar pairing) used to vanish the instant YOLO/
#       ByteTrack missed a single frame — there was no persistence for them at all. Now their last
#       box is held on screen briefly, unless it was already at the frame edge or too small.
#
#  Launch:  python fusion-python-3.10_persistent.py                          — live camera + radar (or --dump x.bin)
#           python fusion-python-3.10_persistent.py --selftest --dump x.bin   — no camera: camera emulated from radar data
#
#  Radar — via iwr1642_live_persistent only (tracks with IDs). No custom TLV parsing here.
# =====================================================================
import argparse
import os
import csv
...
import radar_filter_persistent as radar_filter
...
HOLD_MIN_BBOX_PX = 14                # a radar-held box that has scaled down below this height on extrapolation is too far/small to keep trusting — treated as out-of-frame (dropped) instead of shown as a sliver
CAM_ONLY_HOLD_S = 1.5                # camera-only (no radar pairing) detections: keep drawing the last known box this long after YOLO/ByteTrack stops reporting the id, so one or two missed frames don't make it vanish
CAM_ONLY_MIN_BBOX_PX = 10            # ...unless that last box was already this small (too far to trust)
CAM_ONLY_EDGE_MARGIN_PX = 2          # ...or already touching the frame edge (the object left the frame)
...
class CamDet:
    ...
    obj_h_m: float = 1.7
    held: bool = False   # True — synthetic entry from CamOnlyHold (YOLO didn't report this id this frame; last known box shown for continuity)
    ...
    def on_radar(self, t, radar_frame, tracks):
        ...
        for i, tr in enumerate(tracks):
            ...
            if state == "both":
                ...
                if (bbox[3] - bbox[1]) < HOLD_MIN_BBOX_PX:
                    # extrapolated box has shrunk to a sliver — too far / too small to keep trusting,
                    # treat like leaving the frame rather than holding an unreadable box forever
                    state, bbox = "out-of-frame", None
                else:
...
class CamOnlyHold:
    """Keeps drawing a camera-only (no radar pairing) detection at its last known box for
    CAM_ONLY_HOLD_S after YOLO/ByteTrack stops reporting its id, so one or two missed frames don't
    make the object vanish — the same "don't disappear immediately" rule the radar side gets via
    Fusion's own hold state, just without radar range to extrapolate from (so the box is held
    static, not scaled). Stops holding — and lets the object disappear — the moment its last box
    was already touching the frame edge (left the frame) or had shrunk below CAM_ONLY_MIN_BBOX_PX
    (too far / too small to trust), or once the hold window itself expires.

    Lives in the main camera-loop thread only (run_live's single-threaded per-frame loop) — keep
    it off Fusion, which is read from the radar thread too."""

    def __init__(self):
        self.mem = {}   # cam_id -> {"bbox", "cls", "conf", "last_t"}

    def update(self, t, cam_dets, fused_cam_ids, frame_w, frame_h):
        present = set()
        for d in cam_dets:
            if d.cam_id in fused_cam_ids:
                continue
            present.add(d.cam_id)
            self.mem[d.cam_id] = {"bbox": (d.x1, d.y1, d.x2, d.y2), "cls": d.cls, "conf": d.conf, "last_t": t}
        held = []
        for cam_id in list(self.mem):
            if cam_id in present or cam_id in fused_cam_ids:
                continue
            m = self.mem[cam_id]
            x1, y1, x2, y2 = m["bbox"]
            out_of_frame = x1 <= CAM_ONLY_EDGE_MARGIN_PX or x2 >= frame_w - CAM_ONLY_EDGE_MARGIN_PX or \
                y1 <= CAM_ONLY_EDGE_MARGIN_PX or y2 >= frame_h - CAM_ONLY_EDGE_MARGIN_PX
            too_small = (y2 - y1) < CAM_ONLY_MIN_BBOX_PX
            expired = (t - m["last_t"]) > CAM_ONLY_HOLD_S
            if out_of_frame or too_small or expired:
                del self.mem[cam_id]
                continue
            held.append(CamDet(m["last_t"], cam_id, m["cls"], m["conf"], x1, y1, x2, y2, held=True))
        return held
...
def draw_overlay(frame, fused, cam_dets, matched_idx, cam: CameraModel, tracks, radar_stale=False, fps=None):
    ...
    thin orange — camera only (range from bbox height, "≤" if the box is clipped by the edge; dashed —
    YOLO missed this frame, CamOnlyHold is briefly holding its last box); red circle — moving radar with
    no pair. The number on the box is the range to the nearest point (radar)."""
    img = frame.copy()
    if fps is not None:
    ...
    for d in cam_dets:
        ...
        if d.held:
            _dashed_rect(img, (d.x1, d.y1), (d.x2, d.y2), (0, 140, 255), 1)
            label = f"{d.cls} (camera only, holding)"
        else:
            cv2.rectangle(img, (d.x1, d.y1), (d.x2, d.y2), (0, 140, 255), 1)
            label = f"{d.cls} (camera only)"
        cv2.putText(img, label, (d.x1, max(d.y1 - 30, 12)), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 140, 255), 1)
...
def run_live(dump=None):
    ...
    try:
        while True:
            ...
            fus.push_camera(t, dets, frame)      # matching sees only real detections, never held/synthetic ones
            n += 1
            fusion_fps.tick()
            if n % 50 == 0:
            ...
            fused_cam_ids_now = {f["cam_id"] for f in s["fused"] if f.get("state") == "both"}
            draw_dets = dets + cam_hold.update(t, dets, fused_cam_ids_now, w, h)
            if SHOW_WINDOW:
                img = draw_overlay(frame, s["fused"], draw_dets, s["matched"], cam, s["tracks"], radar_stale=stale,
