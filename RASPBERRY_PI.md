# Running this project on a Raspberry Pi 4

This branch (`claude/raspberry-pi-4`) is the Pi-4 configuration of the project: the same
radar + camera fusion pipeline, with `configs.json` and the YOLO loading path (`fusion-python-3.10.py`)
set up for what's actually achievable on a Pi 4's CPU — no CUDA, and YOLO run through the NCNN backend
at a reduced input resolution instead of stock PyTorch at 640×640.

**Not yet validated on real Pi 4 hardware.** Everything here was checked by: compiling every changed
file, running targeted unit tests against stubbed `ultralytics`/`torch` modules to verify the control
flow (export-once/cache/reload, kwargs passed to `model.track()`), confirming real aarch64 wheel
availability for every dependency against the live PyPI index, and reasoning from the FPS analysis
done earlier in this project. It has **not** been run on an actual Raspberry Pi with the real radar
and camera attached — treat the performance numbers below as estimates, and expect to spend time on
serial port names, camera index, and USB power in particular (see below).
...
- **Stuck on Bullseye (Python 3.9):** pin `torch<=2.2.2` and `torchvision<=0.17.2` (the last releases
  ...
  rest. `ultralytics` only requires `torch>=1.8.0`, so this is compatible.
...
- **`USE_NCNN`** (`configs.json`, default `1`) — runs YOLO through Ultralytics' NCNN backend instead of
  stock PyTorch. This is the one change that makes the CV part of the pipeline viable on a Pi 4's CPU;
  see the FPS estimate below. `USE_GPU` is left wired up but harmless — there's no CUDA GPU on a Pi 4,
  so it always falls back to CPU regardless of that setting (default set to `0` here just to skip the
  pointless `torch.cuda.is_available()` warning at startup).
...
- **`DETECTOR`** defaulted to `"yolov8n"` instead of `"yolo-world"` — the open-vocabulary detector's
  CLIP text tower is too slow for a Pi 4 even before considering NCNN (see the earlier cost analysis:
...
Carried over from the earlier cost analysis in this project (YOLOv8n + NCNN + 320×416, "ignore all
other optimizations"): an estimated **~10–18 FPS**, most likely around **12–15 FPS**, on the camera/
fusion side. This is extrapolated from published Ultralytics/community Pi 4 benchmarks and FLOP-scaling
with resolution — not a direct measurement, since no Pi 4 is available to benchmark against in this
environment. Actual results will depend on your specific NCNN build, thermal throttling under sustained
load, and USB camera capture overhead. The radar-processing and sensor-fusion math (EKF, DBSCAN, GNN
association) are not a bottleneck at any resolution — they were already estimated at low-single-digit
milliseconds per frame on much weaker hardware than a Pi 4's Cortex-A72.

## 7. Power

A Pi 4 running sustained YOLO inference on all 4 cores, plus a USB camera and a USB-connected radar
board, can draw more than older/cheaper "phone charger" USB power supplies provide. Use the official
5V/3A USB-C supply (or better), and if you see random USB disconnects (the radar or camera dropping
out), that's the first thing to check — not a code bug.
...
- [ ] Measure real FPS and compare against the ~12–15 FPS estimate above
