...
#
#  PERSISTENT-TRACKING VARIANT (copy of iwr1642_live.py): a track is only dropped when it's
#  genuinely gone — too far away to trust (beyond the radar's own max range) or coasting on
#  prediction for far longer than before — instead of after a handful of missed frames. See
#  TRACK_COAST_MISSES / TRACK_MAX_RANGE_M below. Everything else is unchanged from
#  iwr1642_live.py. Use together with fusion-python-3.10_persistent.py.
#
...
TRACK_COAST_MISSES = 50                      # frames a track survives on prediction alone before being dropped as genuinely gone (5 s at 10 Hz) — generous on purpose, so a brief radar dropout doesn't make an object vanish
TRACK_MAX_RANGE_M = None                     # objects beyond this range are "too far / too small to reliably detect" and are dropped immediately regardless of TRACK_COAST_MISSES; None — use the .cfg's own max_range_m (set by Pipeline)
...
class Tracker:
    def __init__(self, confirm_hits=TRACK_CONFIRM_HITS, confirm_window=TRACK_CONFIRM_WINDOW,
                 max_misses=TRACK_COAST_MISSES, max_range_m=TRACK_MAX_RANGE_M, gate=3.5, merge_m=0.5):
        self.confirm_hits, self.confirm_window, self.max_misses = confirm_hits, confirm_window, max_misses
        self.max_range_m = max_range_m           # "too far / too small to detect" cutoff — drops a track immediately, bypassing max_misses
    ...
    def step(self, objs, t, dt):
        ...
        for tr in self.tracks:
            ...
            dead = tr.misses > self.max_misses or \
                (not tr.confirmed and tr.age > self.confirm_window and tr.hits < self.confirm_hits) or \
                (self.max_range_m is not None and tr.range_m > self.max_range_m)   # too far / too small to trust — drop now, don't coast toward it
            if not dead:
                alive.append(tr)
...
class Pipeline:
    def __init__(self, cfg, model=None, ego_speed=0.0, use_background=USE_BACKGROUND):
        ...
        max_range = TRACK_MAX_RANGE_M if TRACK_MAX_RANGE_M is not None else cfg.get("max_range_m")
        self.tracker = Tracker(max_range_m=max_range)
